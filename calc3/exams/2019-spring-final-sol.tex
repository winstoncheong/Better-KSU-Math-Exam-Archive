...
\begin{solution}
    ...
    \begin{align*}
        D_{(2, 0)} &= 24 > 0 \quad \text{and}\quad f_{xx}(2, 0) = 12 > 0 \quad \text{ so } (2, 0) \text{ is a local min} \\ 
        D_{(-2, 0)} &= -24 < 0 \quad \text{so } (-2, 0) \text{ is a saddle point}
    \end{align*}
\end{solution}
...
\begin{solution}
    Let $\gamma_1, \gamma_2, \gamma_3, \gamma_4$ denote paths along the right, left, top, and bottom sides of the square $\mathcal{D}$.
