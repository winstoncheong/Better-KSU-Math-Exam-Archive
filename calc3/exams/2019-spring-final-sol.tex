...
\begin{parts}
	...
	\]
	\begin{solution}
		To make this match part (c), $\mathcal S$ consists only of the lateral surface of the cylinder.
		This surface can be parametrized by 
		\[
			G(\theta, z) = (\cos \theta, \sin\theta, z), \quad 0 \le \theta \le 2\pi, \ 0 \le z \le 3
		\]
		with outward pointing normal vector
		\[
			\vb{N} = G_\theta \cross G_z = \vect{-\sin\theta, \cos\theta,0} \cross \vect{0, 0, 1} = \vect{\cos\theta, \sin\theta, 0}
		\]
		Hence the vector surface integral can be calculated directly as
		\begin{align*}
			\iint_{\mathcal S} \tcurl(\vb{F}) \vdot \dif \vb{S}
			&= \int_0^3 \int_0^{2\pi} \vect{\cos\theta, \sin\theta, -2z} \vdot \vect{\cos\theta, \sin\theta, 0} \dif \theta \dif z \\ 
			&= \int_0^3 \int_0^{2\pi} 1 \dif \theta \dif z
			= \boxed{6\pi}
		\end{align*}
	\end{solution}
	...
	\begin{solution}
		By Stokes' Theorem, 
		\[
			\iint_{\mathcal S} \tcurl(\vb{F}) \vdot \dif \vb{S} = \int_{\partial \mathcal{S}} \vb{F} \vdot \dif \vb{r}.
		\]
		Using the result from part (b) and the fact that the contour integral over $\mathcal{C}_1$ is zero, we have
		\[
			\int_{\mathcal C_2} \vb{F} \vdot \dif \vb{r} = \boxed{6\pi}
		\]
	\end{solution}
\end{parts}
\end{questions}
