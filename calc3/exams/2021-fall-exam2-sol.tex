...
&= \lim_{r\to0} \frac{\sin \theta (r\cos \theta - 1)}{r^2 \cos \theta} \\ 
    \end{align*}
    This function still depends on $\theta$, so the limit \fbox{does not exist}
\end{solution}
\part[5]
\[
...
\begin{alignat*}{2}
              &                 & f(-1, 1)   & = 2  \\
    f_x(x, y) & = y -2          & f_x(-1, 1) & = -1 \\
    f_y(x, y) & = x + 2y \qquad & f_y(-1, 1) & = 1
\end{alignat*}
...
\begin{align*}
    ...
    &= 2 \sqrt 3 \cdot \frac{\sqrt 3}{3} \cdot 3 = \boxed{6}
\end{align*}
\end{solution}
