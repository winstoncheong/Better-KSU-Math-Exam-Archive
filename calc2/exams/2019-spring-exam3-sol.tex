...
\begin{parts}
    ...
    \begin{solution}
        ...
        \end{align*}
        \textit{Remark:}
        Since $x = \sinh(\theta)$, we can use the hyperbolic pythagorean identity to get $\cosh \theta = \sqrt{1+\sinh^2 \theta} = \sqrt{1+x^2}$.
    \end{solution}
\end{parts}
...
\]
\begin{solution}
    The expression in absolute values is bounded above by the $(M+1)$-st~term in the series. Thus we want
    \begin{align*}
...
\begin{parts}
    ...
    \begin{solution}
        Since
        \[
            \sum_{n=1}^\infty \left| \frac{(-1)^{n+1}\sin(n)}{5e^n}\right|
            = 
            \sum_{n=1}^\infty \frac{|\sin(n)|}{5e^n} 
            \le 
            \sum_{n=1}^\infty \frac{1}{5e^n} 
        \]
        and the right series converges, being a geometric series with $r = \frac1e \approx \frac{1}{2.7} < 1$, by the Direct Comparison Test, the original series \fbox{converges absolutely}
    \end{solution}
\end{parts}
\end{questions}
\end{document}